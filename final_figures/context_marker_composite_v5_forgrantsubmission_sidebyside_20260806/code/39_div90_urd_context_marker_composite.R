...
cluster_tree_plot <- function(layout, cells, mapping, colors, show_fate_bars = FALSE, large_labels = FALSE) {
  ...
  if (large_labels && nrow(labels) > 0) {
    ...
    labels$label <- paste0(labels$cluster_id, " - ", wrapped(label_names, 22))
  }
  ...
  if (show_fate_bars && nrow(labels) > 0) {
    ...
    fate <- rbind(
      data.frame(
        fate = "Neurogenesis",
        y_min = min(labels$y[labels$cluster_id %in% c(1:7, 9)]) - 0.18,
        y_max = max(labels$y[labels$cluster_id %in% c(1:7, 9)]) + 0.18
      ),
      data.frame(
        fate = "Gliogenesis",
        y_min = min(labels$y[labels$cluster_id %in% c(8, 10)]) - 0.18,
        y_max = max(labels$y[labels$cluster_id %in% c(8, 10)]) + 0.18
      )
    )
...
simplified_lineage_tree_plot <- function(layout, cells) {
  df <- cells
  df$lineage_context <- ifelse(
    df$published_cluster_id == 7,
    "Cortical lineage (cluster 7)",
    ifelse(
      df$published_cluster_id %in% 1:6,
      "Other neuronal lineages (clusters 1–6)",
      ifelse(
        df$published_cluster_id %in% c(8, 10),
        "Gliogenic branches (clusters 8, 10)",
        "Dividing cells (cluster 9)"
      )
    )
  )
  levels <- c(
  ...
  df$lineage_context <- factor(df$lineage_context, levels = levels)
  df <- df[order(df$lineage_context == "Cortical lineage (cluster 7)"), , drop = FALSE]
  colors <- c(
    "Other neuronal lineages (clusters 1–6)" = "#587681",
    "Cortical lineage (cluster 7)" = "#B08DB0",
    "Gliogenic branches (clusters 8, 10)" = "#D7D7D7",
    "Dividing cells (cluster 9)" = "#AFAFAF"
  )
  ...
  lim <- tree_limits(layout, df, right_extra = 0.08)
  x_range <- range(c(layout$x1, layout$x2, df$x), na.rm = TRUE)

  ggplot() +
    base_tree_plot(layout) +
    geom_point(data = df, aes(x = x, y = y, color = lineage_context), size = 0.32, alpha = 0.88) +
    annotate(
...
marker_tree_plot <- function(
  ...
  p <- ggplot() +
    ...
    scale_color_gradientn(
      ...
      oob = scales::squish,
      name = if (compact_panel) "log1p\n(CP10K)" else "log1p(CP10K)",
    ...
    theme(
      ...
      plot.margin = if (compact_panel) margin(1, 1, 1, 1, unit = "pt") else margin(5.5, 5.5, 5.5, 5.5),
      legend.key.height = grid::unit(if (compact_panel) 25 else 14, "pt"),
      legend.key.width = grid::unit(5, "pt"),
      ...
      legend.title = element_text(size = if (compact_panel) 8 else 6),
      legend.text = element_text(size = if (compact_panel) 8.5 else 5)
    ) +
...
marker_plots <- setNames(lapply(spec$gene, function(gene) {
  marker_tree_plot(
    ...
    lineage_guides = marker_ctx$lineage_guides,
...
render_status <- data.frame(
  ...
  value = c(
    ...
    if (cfg$layout_style == "grant_side_by_side") {
      "single lower row; module 1 width 42%; module 2 width 58%; vertical black module strips; bordered module containers"
    } else if (cfg$layout_style == "grant_modules") {
