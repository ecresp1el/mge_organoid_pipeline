...
def verify_existing(output_root, script_path, lock_path):
    ...
    environment_path = os.path.join(output_root, "software_environment.tsv")
    with open(environment_path, "r", newline="") as handle:
        environment = {row["key"]: row["value"] for row in csv.DictReader(handle, delimiter="\t")}
    if environment.get("pipeline_script_sha256") != sha256_file(script_path):
        fail("Existing X-GFP audit was generated by a different pipeline script")
    if environment.get("reference_lock_sha256") != sha256_file(lock_path):
        fail("Existing X-GFP audit was generated from a different reference lock")
    return True
...
def run(args):
    ...
    if verify_existing(output_root, script_path, args.lock):
    ...
    try:
        ...
        staging = None
        verify_existing(output_root, script_path, args.lock)
