...
class Step03ResultValidator:
    ...
    def validate(self, adata: ad.AnnData, results: pd.DataFrame, original_fingerprint: str) -> None:
        """Validate cell identity, one scores-only result, calls, and raw counts."""

        required = {"cell_id", "primary_score", "primary_class"}
        missing = sorted(required - set(results.columns))
        self.ledger.add("r_result_fields", "result", not missing, missing or "none", "none", "The single native-R invocation must return one score and one singlet/doublet annotation per cell.")
        unexpected = sorted(set(results.columns) & {"primary_cluster", "replicate_score", "replicate_class"})
        self.ledger.add("single_invocation_schema", "result", not unexpected, unexpected or "none", "none", "The promoted implementation must not imply unavailable full-return or second-seed outputs.")
        ...
        values = results["primary_score"].to_numpy(dtype=float)
        valid = bool(np.isfinite(values).all() and np.logical_and(values >= 0, values <= 1).all())
        self.ledger.add("primary_score_range", "result", valid, {"min": float(np.nanmin(values)), "max": float(np.nanmax(values))}, "finite [0,1]", "scDblFinder scores must be finite and within [0,1].")
        observed = sorted(results["primary_class"].astype(str).unique())
        self.ledger.add("primary_class_values", "result", set(observed) == {"singlet", "doublet"}, observed, ["doublet", "singlet"], "Both annotation classes must be represented and no undocumented value is allowed.")
        observed_fingerprint = Step02MatrixFingerprint.memory(adata.X)
        self.ledger.add("raw_matrix_unchanged", "object", observed_fingerprint == original_fingerprint, observed_fingerprint, original_fingerprint, "Attaching annotations must not alter a single raw count.")
