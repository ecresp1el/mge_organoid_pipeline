    ...
    def _report(self, samples: Sequence[Mapping[str, object]], inventories: Mapping[str, Sequence[Mapping[str, object]]]) -> str:
        ...
        return "\n".join([
            ...
            *sample_lines,
            ...
            "- Later atlas: the Mayer lab's 2025 interactive atlas uses a local `EXCIT_INHIBIT_cleaned_sub.rds` containing Bandler embryonic cells, stages, UMAP, classes, clusters, study, and cell IDs. Its intended public vector plots are captured by standalone stage `00f`; standalone stage `00g` independently recovers deposited E15 barcodes from public RNA-expression fingerprints and preserved order, although the underlying RDS/generated cell table is not exposed through an intended endpoint.",
            "",
            "Therefore CA301 remains the strongest anatomical/age match. The original 21 published embryonic labels must not be assigned without an original author barcode table; the separate later-MIND barcode recovery is explicitly a later reanalysis taxonomy.",
            "",
