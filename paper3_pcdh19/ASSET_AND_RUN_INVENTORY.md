...
| `00_developing_mouse_mge_reference_curation` | Published processed objects plus published and object-linked sample/library metadata for La Manno 2021, Bandler 2022, and Mayer 2018 | First checkpoint complete on 2026-08-29; Bandler follow-ups recovered postnatal STICR, official supplements, public later-atlas vectors, and the deposited E15 barcode join | Bandler supplies 4,516 WT E15.5 MGE count-matrix cells. Of the 7,420 Bandler E15 cells retained in the later atlas, 4,481 are definitively recovered CA301 MGE barcodes, 2,937 are CA302 CGE, and 2 are CA303 LGE, with later-atlas labels/plot coordinates. Original 2022 per-cell labels remain unavailable. Mayer supplies 6,515 MGE E13.5 Lhx6-positive cells; La Manno E15 MGE membership is unproven. Stop before final selection or Paper 3 mapping. |
...
| `primary_processing/00_input_validation_and_canonical_anndata` | Canonical sparse raw-count AnnData from 12 per-sample filtered Cell Ranger matrices | Run `00_input_validation_and_canonical_anndata_20260830_113749_d8b6bf7`; job `59279775` complete; 221/221 checks pass; explicitly `APPROVED` 2026-08-30 | 450,788 cells × 19,071 genes; 1,295,361,777 nonzeros; raw integer counts in `.X`; no filtering, normalization, reduction, clustering, annotation, or downstream-label join. |
| `primary_processing/01_qc_metrics` | Approved Step 00 counts plus Scanpy QC metadata | Run `01_qc_metrics_20260830_115715_2b57907`; job `59281063` complete; 40/40 checks pass; explicitly `APPROVED` 2026-08-30 | 450,788 cells × 19,071 genes retained with exact sparse-matrix fingerprint; 15 PNG plus 15 PDF diagnostics; no thresholds, filtering, normalization, reduction, clustering, or annotation. |
| `primary_processing/01a_qc_mad_sensitivity` | Exact Step 01 cell-level QC metadata | Review-target run `01a_qc_mad_sensitivity_20260830_121931_d5936f9`; job `59282437` complete; 71/71 checks pass; explicitly `APPROVED` 2026-08-30 | Per-technical-sample 3/4/5 scaled-MAD any-candidate rates 4.765%/2.024%/0.985%, dominated by the mitochondrial tail; 30 figures and complete per-cell/summary/overlap tables; no design grouping, upper count/gene rules, filtering, or replacement H5AD. |
| `primary_processing/02_qc_filtering` | Approved Step 01 raw counts plus approved Step 01a 5-MAD flags | Run `02_qc_filtering_20260830_124611_97e1bb5`; job `59287494` complete; 38/38 checks pass; explicitly `APPROVED` 2026-08-30 | Excluded 4,439 reviewed cells with exact reasons; retained 446,349 cells × all 19,071 genes as raw integer counts; six review figures; no doublet detector ran. |
| `primary_processing/03_scdblfinder` | Approved Step 02 raw integer counts | Job `59389340` completed scDblFinder but wrapper finalization failed because split-sample `returnType=full` was ignored; diagnostic job `59396464` was user-cancelled after 22:27 with logs preserved; no Step 03 result was published | Permanent implementation promoted to one observable serial `returnType=scores` call with the same capture/model/seed settings; preserve all scores/calls, remove no cells, and submit only after review. |
...
| Reference-curation implementation | Main curation, Bandler recovery/inspection/supplement publishing, public-atlas capture/vector counting, expression-fingerprint barcode recovery, hierarchy/report scripts, `config/developing_mouse_mge_*`, submit/status wrappers, and `slurm/00a-00g_*.sbatch` | Object-oriented registry/cache, metadata parsing, H5AD/RDS/CSV inspection, reconciliation, intended public Shiny capture, vector validation, ordered fingerprint matching, artifact-scope evaluation, supplement extraction, plotting, and report publication. It records published versus object-linked IDs/counts, exact observed/later-atlas labels, explicit gaps, and barcode confidence. Submission freezes executable copies and supports confined same-step overwriting. |
