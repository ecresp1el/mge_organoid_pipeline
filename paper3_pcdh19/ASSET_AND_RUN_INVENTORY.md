...
| `primary_processing/02_qc_filtering` | Approved Step 01 raw counts plus approved Step 01a 5-MAD flags | Run `02_qc_filtering_20260830_124611_97e1bb5`; job `59287494` complete; 38/38 checks pass; `IN_REVIEW` | Excluded 4,439 reviewed cells with exact reasons; retained 446,349 cells × all 19,071 genes as raw integer counts; six review figures; no doublet detector ran. |
| `primary_processing/03_scdblfinder` | Future approved Step 02 raw integer counts | Capture definition proposed; computation not authorized | Proposed `samples` unit is the one physical `GEX_1` capture, not 12 Probe Barcode samples; scDblFinder has not run. |
