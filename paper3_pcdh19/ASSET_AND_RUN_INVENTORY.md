...
| `00_developing_mouse_mge_reference_curation` | Published processed objects plus published and object-linked sample/library metadata for La Manno 2021, Bandler 2022, and Mayer 2018 | First checkpoint complete on 2026-08-29; all replacement-run jobs passed and the report/dictionaries are manifested | La Manno has embedded labels/UMAP/tSNE; Bandler and Mayer P0 files are counts-only. Stop before annotation reconstruction, final selection, or Paper 3 mapping. |
...
| Reference-curation implementation | `scripts/reference_curation/developing_mouse_mge_reference_curation.py`, `scripts/reference_curation/inspect_bandler_rds.R`, `scripts/reference_curation/generate_reference_curation_report.py`, `config/developing_mouse_mge_*`, `bin/submit_developing_mouse_mge_reference_curation.sh`, `bin/status_developing_mouse_mge_reference_curation.sh`, and `slurm/00a-00c_developing_mouse_mge_*.sbatch` | Object-oriented registry/cache, GEO/author sample-metadata parsing, H5AD/RDS/CSV inspectors, per-study reconciliation, checkpoint publisher, and observed-object report builder. It records published versus P0-linked sample IDs/counts, biological and library fields, exact observed P0 labels, explicit gaps, and the next-stage requirements ledger. Submission freezes executable copies into a versioned run and supports guarded same-step replacement. |
