...
| `primary_processing/03_scdblfinder` | Approved Step 02 raw integer counts | One-`GEX_1`-capture definition explicitly `APPROVED` 2026-08-30; object-oriented Python/native-R code, frozen submit package, environment contract, and review diagnostics implemented; computation pending | Use `clusters=TRUE`, `dbr.sd=1`, no manual `dbr`, and otherwise model defaults; run a second-seed reproducibility pass; preserve scores/calls/internal PCA and remove no cells before review. |
