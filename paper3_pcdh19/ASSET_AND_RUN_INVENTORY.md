...
| `primary_processing/02_qc_filtering` | Approved Step 01 raw counts plus approved Step 01a 5-MAD flags | Authorized and implemented; Great Lakes run pending | Will exclude exactly 4,439 reviewed cells while preserving every reason, retain 446,349 cells × 19,071 genes as raw integer counts, publish sample/design summaries, then stop `IN_REVIEW`; no doublet detector runs. |
