...
timed_step("save preprocessed CDS RDS", {
...
if (!has_umap(cds)) {
  ...
  timed_step("save UMAP checkpoint", {
    saveRDS(cds, CDS_UMAP_CLUSTER_RDS)
  })
} else {
...
if (!has_umap_clusters(cds)) {
  ...
  timed_step("save UMAP + cluster checkpoint", {
    saveRDS(cds, CDS_UMAP_CLUSTER_RDS)
  })
} else {
...
timed_step("save UMAP + cluster checkpoint before learn_graph", {
  saveRDS(cds, CDS_UMAP_CLUSTER_RDS)
})
...
timed_step("save UMAP/cluster/graph CDS RDS", {
