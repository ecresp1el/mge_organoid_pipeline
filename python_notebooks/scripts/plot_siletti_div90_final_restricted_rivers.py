...
def main() -> None:
    ...
    (reports_dir / "README_final_candidate_restricted_rivers.md").write_text(
        ...
        "selected reference` is retained for reference accounting and excluded "
        "from transfer training.\n\n"
        ...
        "followed by derived `unified_major_subtype_roi` and "
        "`unified_pallial_subpallial_bin` plotting labels. fast-kNN uses shared "
        "unique genes, library-size normalization to 10,000 counts/cell, log1p, "
        "top 3,000 variable genes, TruncatedSVD with 50 components, cosine kNN "
        "with k = 50, and normalized cosine-similarity weighted voting.\n\n"
